...
- V3 까지만 해도 돼.
- V3는 페치조인으로 한방에 관련된 엔티티를 다 가지고 오는건데, V4는 엔티티를 가지고 올때 딱 DTO에 맞는 필드들만 가져와서 new 키워드를 사용해서 DTO를 생성까지 해서 반환시키는거.
...
#### 주문 조회 V1: 엔티티 직접 노출
- 맞네 지금껏 @ManyToOne, @OneToOne에 대한것을 해왔는데, 이제 @OneToMany에 대한걸 할꺼야.
- 내 예시로 보면 List<Talk> talks = friend.getTalks(); 해서 talks를 뽑아낸다음에. 일단 얘는 프록시가 들어가있겠지. 근데 문제는 talks를 인자로 넘긴기든 어쨌든 결국 talks 가지고 다시 내가 for문을 돌리고 있어.
- for(Talk talk: talks) 문을 돌리면서 talk.getSpeaker()를 하면서 얘가 사람이나 관리자냐 ai 냐에 따라 메시지를 다르게 등록하는 로직이 있는데, 이거 get 할때마다 새로운 쿼리가 튀어나갈거잖아.
- 이러면 만약 talks이 100개다....쿼리 100개.. 이야..이거 너무 끔찍한데,,  톡 하나 주고 받을때마다 쿼리 100개....ㅋㅋㅋㅋㅋㅋ
  
#### 주문 조회 V2: 엔티티를 DTO로 변환
- 엔티티를 그대로 노출하지 말라는것은 DTO로 변환해서 반환하라는 거잖아.
- 근데 DTO 안에 엔티티 필드가 있으면 이 엔티티도 또 DTO로 변환해야되는거야.
- 그냥 엔티티가 필드로 있는데 그걸 DTO로 감싼다고 해서 되는게 아니지 당연히. 근데 이런 실수들을 실무에서도 많이 한대. 주의
  
#### 주문 조회 V3: 엔티티를 DTO로 변환 - 페치 조인 최적화
- 결국 @OneToMany에서도 쿼리가 여러번 안나가게 하려면 필요한 애들을 한번에 퍼오는 페치조인을 해줘야돼.
- 대신 @ManyToOne 이나 @OneToOne에서와 다르게 1:다 관계에서는 조인을 하게 되면 데이터가 뻥튀기 되기 때문에(다 쪽만큼 데이터 개수가 늘어나기에) distinct 키워드를 쿼리문에 넣어야 되고.
- 이 distinct 키워드는 db에서는 원래 모든 필드값이 동일해야 중복제거가 되는데, 여기에서는 루트 테이블을 기준으로 해당 엔티티의 id값만 동일하면 중복 제거해줌.
- 데이터 뻥튀기 현상때문에 페이징처리 하면 안돼. 억지로 하면 하이버네이트가 뻥튀기된 데이터를 메모리에 올린다음에 페이징 처리를 해주는데,, 건수가 10000건이면 아주 큰일나겠지..
- 1:다 정도는 이렇게 distinct 페치조인으로 해결한다 쳐도 여기서 멈춰야지 1:다:다 이런식으로 막 페치조인하면 큰일남.
  
#### 주문 조회 V3.1: 엔티티를 DTO로 변환 - 페이징과 한계 돌파
- 결국 위의 방법은 페치조인으로 데이터뻥튀기 된거 까지 다 퍼온다음에 중복제거하는거.
- 쿼리 수 한방에 다 가져올 수 있는 방법이긴 하지만, 일단 페이징이 안되고, 중복 데이터로 인한 db에서의 데이터 전송양이 많음.
- 따라서 일단 XXXToOne 관계만 페치조인 해주고, 그 다음에 OneToMany 관계인것끼리는 default_batch_fetch_size 로 글로벌 설정을 주거나 또는 @BatchSize 를 필드레벨에 선언해서 개별 필드 설정 줌
- 해당 옵션은  컬렉션이나, 프록시 객체를 한꺼번에 설정한 size 만큼 IN 쿼리로 조회하게 함.
- ToOne 관계는 페치 조인해도 페이징에 영향을 주지 않는다. 따라서 ToOne 관계는 페치조인으로 쿼리 수를 줄이고 해결하고, 나머지는 hibernate.default_batch_fetch_size 로 최적화 하자.
- 보통 사이즈는 100~1000
- 데이터 뻥튀기 문제도 해결 됐으니, 페이징도 가능해짐.
- 다만, 위의 방법보다는 쿼리가 몇개 더 나갈 수 있긴한데, 사실 더 정규화된 데이터를 가져오는 느낌이지. 
  
#### 주문 조회 V4: JPA에서 DTO 직접 조회
- 간단한 주문 조회 V4에서 한것처럼 new키워드를 사용하여 딱 필요한 필드에 맞춘 DTO 상태로 조회하는거
- 최적화 안해서 N + 1 문제 발생
- 생략. V3.1까지만 해도 될듯.
  
#### 주문 조회 V5: JPA에서 DTO 직접 조회 - 컬렉션 조회 최적화
- V4를 최적화. 그런데 결국 이것도 select 절이 줄어들긴 하는데, 너무 번거로움.
- 생략. V3.1까지만 해도 될듯.
...
#### API 개발 고급 정리
- 결론: 엔티티로 조회하고, xxxToOne 관계는 페치조인 걸고, xxxToMany 관계인건 default_batch_fetch_size 설정해서 가져오는 V3.1 버전쓰자.
  
### API 개발 고급 - 실무 필수 최적화
#### OSIV와 성능 최적화
- OSIV = Open Session In View. 하이버네이트쪽 용어고 JPA에서는 Open EntityManager In View라 칭하긴 하는데 그냥 보통 OSIV라함.
- OSIV on 일때, 기본 설정 값이 true로 되어 있음.
  - 영속성 컨텍스트가 db 커넥션을 트랜잭션이 시작될때 그때 물기 시작해서, 완전히 서버에서 응답이 나갈떄까지 계속 물고 있음.
  - 덕분에, 지연로딩으로 인해 프록시 객체 들어온 것들도 컨트롤러 계층이나 view 쪽에서도 실제 값으로 로딩을 해줌. 지연로딩, 프록시객체 이런것이 영속성 컨텍스트가 해주는거니까.
  - 다만, 이게 너무 오랜시간 db 커넥션을 물고 있다 보니, 요청 들어와서 응답 나갈때까지 뭐 3초 걸린다 치면, 3초 내내 db 커넥션 하나가 하나만 바로보게 되어서, 커넥션 리소스를 많으 쓰게 되고, 그 결과 커넥션이 모자르는 상황이 올수도 있음.
- OSIV off 일때, false로 설정값 따로 주면.
  - 딱 트랜잭션 로직 끝날때, 보통 서비스계층에 있는 메소드가 마무리 될때. db커넥션을 반환하고, 영속성컨텍스트로 닫힘. 더 유연하게 db 커넥션을 사용할 수 있겠지.
  - 다만, 컨트롤러 계층에서 프록시 객체인애들을 로딩 못시키게 되는거야.
  - 때문에 지연로딩 시키는 부분들을 다 서비스 계층에 몰아 넣어서 트랜잭션 선언한 로직 안에서 해결을 봐야함.
- 주로 고객 서비스의 실시간 API 같은건 OSIV를 끄고, admin 같은건 키고.
  
### 다음으로
#### 스프링 데이터 JPA 소개
- public interface MemberRepository extends JpaRepository<Member, Long> { }
- JpaRepository에 이미 모든 CRUD.. 내가 상상할 수 있는 모든게 다 만들어져 있어.
- 인터페이스인데 그러면 구현은? -> Spring Data JPA가 알아서 구현체 생성해서 다 넣어줌.
- 그냥 member.findAll(), member.findById(memberId) 이런거 그냥 다 만들어져있음. 내가 직접 구현할 필요가 아예 없음.
- member.findByName(name); 이런건? 당연히 필드명은 일반화할 수 없는 영역이니까 완전히 만들어져있지 않지만, 해당 인터페이스에다가 그냥
  List<Member> findByName (String name); 이라고 메소드 선언만 해주면, 스프링데이터JPA가 이에 맞춰 구현해서 구현체에 넣어줌.. 즉 그냥 저렇게 메소스 선언만 해주면되는거야.
- 이거 진짜 미쳤는데...
  
#### QueryDSL 소개
- 검색 조건에 따라서 쿼리가 달라지는 동적 쿼리를 짤때. 필요. 근데 이거 그냥 그 setPara인가 그거 줘서 매개변수에 값 입력하는 형태로 쿼리 날리면 되는거 아니었나.
- 동적쿼리짜는데 편하고, JPQL 짤때도 컴파일 시점에서 빨간줄 그어주니까 좋고. 거의 필수라는데 글쎄...아직 필요성이 와닿지 않네.
